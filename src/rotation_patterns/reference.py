"""Fetch and validate the authors' published Figure 1 reference curves.

The reference files are data artifacts, not outputs regenerated by this package.  They
are downloaded from an immutable commit of ``paulyan678/thesis`` and kept separate from
new experiment aggregates.
"""
...
def fetch_reference(config: ExperimentConfig) -> Path:
    ...
    with tempfile.TemporaryDirectory(prefix="reference-", dir=destination.parent) as temp_name:
        ...
        request = Request(REFERENCE_URL, headers={"User-Agent": "rotation-patterns/0.1"})
        try:
